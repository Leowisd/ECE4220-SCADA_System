...
void* periodicUpdate(void *arg)
{
    ...
    while(1)
    {
		while(read(timer_fd, &num_periods,sizeof(num_periods)) == -1); //wait its period
            /******Check Status******/
            sem_wait(&my_sem);
            //button status
...
int main(int argc, char *argv[])
{
    ...
    wiringPiISR(S1,INT_EDGE_FALLING,&switch1_handler);
	wiringPiISR(S2,INT_EDGE_FALLING,&switch2_handler);
